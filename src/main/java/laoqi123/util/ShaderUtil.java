...
public class ShaderUtil {
    ...
    public ShaderUtil(String fragmentShaderLoc, String vertexShaderLoc) {
        ...
        try {
            ...
            if (fragmentShaderLoc.equals("shadow")) {
                ...
                fragmentShaderID = createShader(mc.getResourceManager().getResource(Identifier.of(fragmentShaderLoc)).orElseThrow(() -> new FileNotFoundException(fragmentShaderLoc)).getInputStream(), GL_FRAGMENT_SHADER);
            }
            glAttachShader(program, fragmentShaderID);

            int vertexShaderID = createShader(mc.getResourceManager().getResource(Identifier.of(vertexShaderLoc)).orElseThrow(() -> new FileNotFoundException(vertexShaderLoc)).getInputStream(), GL_VERTEX_SHADER);
    ...
    public static void drawQuads(float x, float y, float width, float height) {
        ...
        bufferBuilder.vertex(x, y, 0.0F).texture(0.0F, 0.0F);
        bufferBuilder.vertex(x, y + height, 0.0F).texture(0.0F, 1.0F);
        bufferBuilder.vertex(x + width, y + height, 0.0F).texture(1.0F, 1.0F);
        bufferBuilder.vertex(x + width, y, 0.0F).texture(1.0F, 0.0F);
    ...
    public static void drawQuads() {
        ...
        bufferBuilder.vertex(0.0F, height, 0.0F).texture(0.0F, 0.0F);
        bufferBuilder.vertex(width, height, 0.0F).texture(1.0F, 0.0F);
        bufferBuilder.vertex(width, 0.0F, 0.0F).texture(1.0F, 1.0F);
        BufferRenderer.draw(bufferBuilder.end());
    }
    ...
    public static void drawFixedQuads() {
        drawQuads((float) (mc.getWindow().getFramebufferWidth() / mc.getWindow().getScaleFactor()), (float) (mc.getWindow().getFramebufferHeight() / mc.getWindow().getScaleFactor()));
    }
