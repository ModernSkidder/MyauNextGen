package laoqi123.module.modules.targethud.impl;
...
public class MyauTargetHUD extends TargetHUDMode {
    ...
    public void render(TargetHud2 targetHUD, TargetHud2.RenderData data, float x, float y) {
        ...
        // OneConfig HUD surface: 50% black scaled by the Background slider, 4px corners.
        int backgroundColor = Glass.alpha(Glass.BG,
                Math.round(this.background.getValue() / 100.0F * 255.0F));
        Glass.panel(0.0F, 0.0F, barTotalWidth, 27.0F, backgroundColor);
        if (this.outline.getValue()) {
            RenderUtil.drawRoundedRectOutline(0.0F, 0.0F, barTotalWidth, 27.0F,
                    Glass.RADIUS, 1.0F, targetColor.getRGB(), true, true, true, true);
        }
        // Health bar as a pill, matching OneConfig's inline controls.
        Glass.bar(headIconOffset + 2.0F, 22.0F, barTotalWidth - 2.0F - (headIconOffset + 2.0F), 3.0F,
                healthBarColor.getRGB(), lerpedHealthRatio);
