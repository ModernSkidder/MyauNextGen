...
import laoqi123.module.modules.targethud.Fonts;
...
public class UnfairTargetHUD extends TargetHUDMode {
    ...
    public void render(TargetHud2 targetHUD, TargetHud2.RenderData data, float x, float y) {
        ...
        if (this.background.getValue() > 0) {
            ...
            // OneConfig HUD surface, sized through this skin's own scale helpers.
            float left = this.scaleX(x, centerX, scale);
            float top = this.scaleY(y, centerY, scale);
            Glass.panel(left, top,
                    this.scaleX(x + width, centerX, scale) - left,
                    this.scaleY(y + height, centerY, scale) - top,
                    Glass.alpha(Glass.BG, backgroundAlpha),
                    this.scaleSize(Glass.RADIUS, scale));
        }
