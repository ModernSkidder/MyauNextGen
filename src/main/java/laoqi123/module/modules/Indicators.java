...
public class Indicators extends Module {
    ...
    public void onRender(Render2DEvent render2DEvent) {
        ...
        for (Entity entity : TeamUtil.getLoadedEntitiesSorted().stream().filter(this::shouldRender).collect(Collectors.toList())) {
            ...
            // Distance label on a OneConfig HUD chip so it stays readable over the world.
            String string = String.format("%dm", (int) mc.player.distanceTo(entity));
            float labelWidth = Glass.textWidth(string);
            float labelHeight = Glass.textHeight();
            float labelX = centerX + offset * x * scaleValue - labelWidth / 2.0f * scaleValue;
            float labelY = centerY + offset * z * scaleValue + 1.0f;
            Glass.panel(labelX - Glass.PAD, labelY - 1.0f,
                    labelWidth + Glass.PAD * 2.0f, labelHeight + 2.0f);
            Glass.text(render2DEvent.getContext(), string, labelX, labelY);
