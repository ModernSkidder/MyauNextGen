...
public class TabGui extends Module {
    ...
    /** Navigation state, exposed so the Compose HUD can render the same selection. */
    public int getLevel() {
        return this.level;
    }
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        // Keyboard handling above must keep running: a HUD is a render layer and never sees
        // key events, so navigation stays here while the Compose HUD draws the result.
        ...
        drawColumn(event.getContext(), font, 0, x, categoryColumnY, CATEGORY_WIDTH, categoryRows, categoryIndex, level == 0, true);
        x += CATEGORY_WIDTH + GAP;
        drawColumn(event.getContext(), font, 1, x, moduleColumnY, MODULE_WIDTH, moduleRows, moduleIndex, level == 1, level >= 1);
        x += MODULE_WIDTH + GAP;
        drawColumn(event.getContext(), font, 2, x, settingColumnY, SETTING_WIDTH, settingRows, settingSelectedRow, level == 2, level >= 2 && !settingRows.isEmpty());
        x += SETTING_WIDTH + GAP;
        drawColumn(event.getContext(), font, 3, x, subColumnY, SUB_WIDTH, subRows, subIndex, level == 3, level >= 3 && !subRows.isEmpty());
    }

    private void handleKeyboard() {
        boolean upPressed = KeyBindUtil.isKeyDown(GLFW.GLFW_KEY_UP);
        boolean downPressed = KeyBindUtil.isKeyDown(GLFW.GLFW_KEY_DOWN);
        boolean leftPressed = KeyBindUtil.isKeyDown(GLFW.GLFW_KEY_LEFT);
        boolean rightPressed = KeyBindUtil.isKeyDown(GLFW.GLFW_KEY_RIGHT);
    ...
    public List<Row> getCategoryRows() {
    ...
    public List<Row> getSettingRows() {
        List<Row> rows = new ArrayList<>();
        Module module = getSelectedModule();
        if (module == null) {
            return rows;
    ...
    private void drawColumn(DrawContext context, Object font, int columnIndex, float targetX, float y, int width, List<Row> rows, int selected, boolean active, boolean visible) {
        ...
        // OneConfig HUD surface, faded in by the open animation.
        RenderUtil.enableRenderState();
        Glass.panel(x, y, width, height, Glass.alpha(Glass.BG,
                (int) (((Glass.BG >>> 24) & 0xFF) * anim.alpha)));
        ...
        for (int i = 0; i < count; i++) {
            ...
            float valueWidth = getStringWidth(font, value);
            String name = limitText(font, row.name, width - TEXT_PADDING * 2.0F - valueWidth);

            drawText(context, font, name, x + TEXT_PADDING, textY, color);
            if (!value.isEmpty()) {
                int valueColor = selectedRow ? Color.WHITE.getRGB() : (row.hasSub ? new Color(205, 205, 205).getRGB() : Color.WHITE.getRGB());
                drawText(context, font, value, x + width - valueWidth - TEXT_PADDING, textY, swapAlpha(valueColor, 255.0F * anim.alpha));
            }
        }
    ...
    private void drawText(DrawContext context, Object font, String text, float x, float y, int color) {
        if (font instanceof UFontRenderer) {
            ((UFontRenderer) font).drawString(text, Math.round(x), Math.round(y), color, false);
        } else {
            context.drawText(mc.textRenderer, text, (int) x, (int) y, color, false);
        }
    }

    private int getStringWidth(Object font, String text) {
        return font instanceof UFontRenderer ? ((UFontRenderer) font).getStringWidth(text) : mc.textRenderer.getWidth(text);
    }

    private float getCenteredTextY(Object font, float rowY) {
        return rowY + (ITEM_HEIGHT - getFontHeight(font)) / 2.0F;
    }

    private int getFontHeight(Object font) {
        return font instanceof UFontRenderer ? ((UFontRenderer) font).getHeight() : mc.textRenderer.fontHeight;
    }
    ...
    public int getSettingsSelectedRow() {
    ...
    private String limitText(Object font, String text, float maxWidth) {
        if (text == null) {
            return "";
        }
        String suffix = "...";
        ...
        if (getStringWidth(font, limited) <= maxWidth) {
            return limited;
        }

        for (int i = kept; i >= 0; i--) {
            String candidate = source.substring(0, i) + suffix;
            if (getStringWidth(font, candidate) <= maxWidth) {
                return candidate;
            }
        }
        return "";
    ...
    public int scrollStart(int selected, int size) {
    ...
    /**
     * One row of a column. Public so the Compose HUD can render the same rows this module
     * computes, keeping keyboard handling here as the single source of truth.
     */
