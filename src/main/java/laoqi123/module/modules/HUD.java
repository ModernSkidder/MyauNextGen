...
public class HUD extends Module {
    ...
    /**
     * The visible, width-sorted module list. Exposed so the Compose ArrayList HUD can
     * render the same set this module computes each tick.
     */
    ...
    private void drawString(DrawContext context, String text, float x, float y, int color, boolean shadow) {
        if (isModernFont()) {
    ...
    private int getScoreboardWidth() {
        if (mc.world == null || !this.scoreboardAvoid.getValue()) {
            return 0;
        }
        ...
        int width = mc.textRenderer.getWidth(objective.getDisplayName());
        for (ScoreboardEntry entry : scoreboard.getScoreboardEntries(objective)) {
            String line = Team.decorateName(scoreboard.getTeam(entry.owner()), Text.literal(entry.owner())).getString();
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (this.chatOutline.getValue() && mc.currentScreen instanceof ChatScreen) {
            String text = ((ChatScreenAccessor) mc.currentScreen).getInputField().getText().trim();
        ...
        if (!this.isEnabled() || mc.getDebugHud().shouldShowDebugHud()) return;

        // The module list itself is normally drawn by the Compose HUD inside OneConfig's
        // Skia scene, where the frosted background lives. Everything above (the chat
        // outline) is unrelated and still drawn here. This stays as the fallback for when
        // OneConfig's UI cannot start.
        if (laoqi123.oneconfig.huds.ArrayListComposeHud.isActive()) return;
    ...
    private void renderStatic(DrawContext context) {
        ...
        if (this.posX.getValue() == 1) {
            x = (float) mc.getWindow().getScaledWidth() - x;
        ...
        if (this.posY.getValue() == 1) {
            y = (float) mc.getWindow().getScaledHeight() - y - height * this.scale.getValue();
        }
        ...
        for (Module module : this.activeModules) {
            ...
            if (this.background.getValue() > 0) {
                // OneConfig HUD surface. Rows stack into one continuous column, so only
                // the outermost edges are rounded; rounding every row would leave notches
                // between neighbours.
                int fill = Glass.alpha(Glass.BG,
                        Math.round(this.background.getValue() / 100.0F * 255.0F));
                boolean firstRow = offset == 0L;
                boolean lastRow = offset == this.activeModules.size() - 1L;
                boolean topRounded = this.posY.getValue() == 0 ? firstRow : lastRow;
                boolean bottomRounded = this.posY.getValue() == 0 ? lastRow : firstRow;
                RenderUtil.drawRoundedRect(bgX1, bgY1, bgX2 - bgX1, bgY2 - bgY1, Glass.RADIUS, fill,
                        topRounded, topRounded, bottomRounded, bottomRounded);
            }
        ...
        if (this.blinkTimer.getValue()) {
            ...
            if (blinkingModule != BlinkModules.NONE && blinkingModule != BlinkModules.AUTO_BLOCK) {
                ...
                if (movementPacketSize > 0L) {
                    ...
                    float blinkX = (float) mc.getWindow().getScaledWidth() / 2.0F / this.scale.getValue()
                            - (float) getStringWidth(blinkText) / 2.0F;
                    float blinkY = (float) mc.getWindow().getScaledHeight() / 5.0F * 3.0F / this.scale.getValue();
                    if (this.shadow.getValue()) {
    ...
    private void renderAnimated(DrawContext context) {
        float scaleVal = this.scale.getValue();
        float scaledWidth = (float) mc.getWindow().getScaledWidth() / scaleVal;
        float scaledHeight = (float) mc.getWindow().getScaledHeight() / scaleVal;
        ...
        long l = System.currentTimeMillis();
        long offset = 0L;
        float deltaTime = 1.0F / Math.max(mc.getCurrentFps(), 5);
