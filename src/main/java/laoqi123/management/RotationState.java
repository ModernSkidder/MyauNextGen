...
public class RotationState {
    ...
    private static float calculateRenderYawOffset(float targetYaw, float currentYawOffset) {
        ...
        double deltaX = RotationState.mc.player.getX() - RotationState.mc.player.prevX;
        double deltaZ = RotationState.mc.player.getZ() - RotationState.mc.player.prevZ;
        ...
        if (RotationState.mc.player.handSwingProgress > 0.0f) {
            newYawOffset = targetYaw;
        }
        float f4 = MathHelper.wrapDegrees(newYawOffset - currentYawOffset);
        float f5 = MathHelper.wrapDegrees(targetYaw - (currentYawOffset += f4 * 0.3f));
        if (f5 < -75.0f) {
            f5 = -75.0f;
        }
        if (f5 >= 75.0f) {
    ...
    public static void applyState(boolean bl, float f, float f2, float f3, int n) {
        state = bl ? 0 : state + 1;
        if (RotationState.mc.player == null) return;
        prevRenderYawOffset = renderYawOffset;
        renderYawOffset = bl ? RotationState.calculateRenderYawOffset(f, renderYawOffset) : RotationState.mc.player.getBodyYaw();
        prevRotationYawHead = rotationYawHead;
        rotationYawHead = bl ? f : RotationState.mc.player.getHeadYaw();
        prevRotationPitch = rotationPitch;
        rotationPitch = bl ? f2 : RotationState.mc.player.getPitch();
        smoothYaw = f3;
        priority = n;
    }
