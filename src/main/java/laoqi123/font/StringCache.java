...
public class StringCache {
    ...
    public int renderString(String str, float startX, float startY, int initialColor, boolean shadowFlag) {
        ...
        float firstVx = Float.MAX_VALUE, firstVy = Float.MAX_VALUE;
        int quads = 0;
        ...
        for (int glyphIndex = 0, colorIndex = 0; glyphIndex < entry.glyphs.length; glyphIndex++) {
            ...
            org.lwjgl.opengl.GL11.glBindTexture(org.lwjgl.opengl.GL11.GL_TEXTURE_2D, texture.textureName);
            ...
            bufferBuilder.vertex(x1, y1, 0).texture(texture.u1, texture.v1).color(r, g, b, a);
            bufferBuilder.vertex(x1, y2, 0).texture(texture.u1, texture.v2).color(r, g, b, a);
            bufferBuilder.vertex(x2, y2, 0).texture(texture.u2, texture.v2).color(r, g, b, a);
            bufferBuilder.vertex(x2, y1, 0).texture(texture.u2, texture.v1).color(r, g, b, a);
            if (firstVx == Float.MAX_VALUE) { firstVx = x1; firstVy = y1; }
            quads++;
        }
        BufferRenderer.drawWithGlobalProgram(bufferBuilder.end());

        if (Boolean.getBoolean("laoqi123.debuggui")) {
            System.out.println("[STRINGCACHE] str='" + str + "' glyphs=" + entry.glyphs.length + " quads=" + quads + " firstQuad=(" + firstVx + "," + firstVy + ") tex=" + (entry.glyphs.length > 0 && entry.glyphs[0].texture != null ? entry.glyphs[0].texture.textureName : -1) + " height=" + this.height + " color=" + String.format("%08X", color));
        }

        if (entry.specialRender) {
            int renderStyle = 0;
            RenderSystem.setShader(ShaderProgramKeys.POSITION_COLOR);
            BufferBuilder bufferBuilder2 = tessellator.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR);            for (int glyphIndex = 0, colorIndex = 0; glyphIndex < entry.glyphs.length; glyphIndex++) {
                while (colorIndex < entry.colors.length && entry.glyphs[glyphIndex].stringIndex >= entry.colors[colorIndex].stringIndex) {
                ...
                if ((renderStyle & ColorCode.UNDERLINE) != 0) {
                    ...
                    bufferBuilder2.vertex(x1, y1, 0).color(0, 0, 0, 255);
                    bufferBuilder2.vertex(x1, y2, 0).color(0, 0, 0, 255);
                    bufferBuilder2.vertex(x2, y2, 0).color(0, 0, 0, 255);
                    bufferBuilder2.vertex(x2, y1, 0).color(0, 0, 0, 255);
                }
