...
public class ConfigMultiChoice extends ConfigOption {
    private final MultiEnumChoiceValue<?> valueProperty;
    ...
    public ConfigMultiChoice(MultiEnumChoiceValue<?> property, int size) {
        super(property, size);
    ...
    public void draw(long vg, int x, int y, InputHandler inputHandler) {
        ...
        EnumSet<?> currentSet = valueProperty.getValue();
        String selected;
        if (currentSet == null || currentSet.isEmpty()) {
            selected = "None";
        } else {
            StringBuilder sb = new StringBuilder();
            for (Object v : currentSet) {
                if (sb.length() > 0) sb.append(", ");
                sb.append(((NamedChoice) v).getChoiceName());
            }
            selected = sb.toString();
        }
        NanoVGRenderUtil.drawText(vg, selected, x + 364, y + 16, Colors.WHITE_80, 14f);
        NanoVGRenderUtil.drawRoundedRect(vg, x + 964, y + 4, 24, 24, Colors.GRAY_500, 8);
    ...
    public void drawLast(long vg, int x, InputHandler inputHandler) {
        ...
        Object[] values = valueProperty.getValues();
        int visible = Math.min(values.length, 10);
        ...
        float maxHeight = values.length * 32 + 8;
        ...
        EnumSet<?> current = valueProperty.getValue();
        float optionY = y + 44 + scroll;
        for (Object value : values) {
            boolean selected = current != null && current.contains(value);
            ...
            if (optionHovered && inputHandler.isClicked(true)) {
                EnumSet newSet = current == null ? EnumSet.noneOf(((Enum) value).getDeclaringClass()) : EnumSet.copyOf(current);
                if (newSet.contains(value)) {
                    newSet.remove(value);
                } else {
                    newSet.add((Enum) value);
                }
                valueProperty.setValue(newSet);
            }
            // checkbox
            NanoVGRenderUtil.drawHollowRoundRect(vg, x + 366, optionY + 7, 18, 18, selected ? Colors.PRIMARY_500 : Colors.WHITE_60, 4, 1.5f);
            if (selected) {
                NanoVGRenderUtil.drawRoundedRect(vg, x + 370, optionY + 11, 10, 10, Colors.PRIMARY_500, 3f);
            }
            NanoVGRenderUtil.drawText(vg, ((NamedChoice) value).getChoiceName(), x + 394, optionY + 18, color, 14);
            optionY += 32;
        }
